...
As @fig-flights-missingness shows below, only `air-time` and `arrival_delays` register more than 0.01% missingness, and both of those still have over 99% valid records.
...
As @fig-missingness_airportcodes shows, it appears that almost 85% of our `airport` data records are missing the `iata_code` values needed to join to our `flights` data.
...
Lastly, after filtering for just U.S. medium and large airports, @fig-missingness_airportcodes_USmedlarge shows that 96% of the `iata_code` records are now valid.
...
As an example, we can see below in @fig-distance_issues that for the `Charlotte-Raleigh` route, the 2 values aren't identical but are close enough to be confident in the original `distance` values.
...
As seen below in @fig-tickets_missingness, only the `ticket_fare` field has at least 0.01% missingness, and even then only a tiny proportion of the `ticket_fare` values are missing.
...
We could calculate an `average_ticket_fare` for each route, but a review of the `ticket_fare` distribution in @fig-ticket_fare_outliers shows significant price variability amongst carriers, along with several major outliers
...
To derive `passenger_counts`, the `occupancy_rates` are multiplied by an assumed max capacity of 200 passengers. These `passenger_counts`in turn serve as multipliers for the `median_ticket_fare` and `baggage_fee` calculations.
...
Further, when just the originating airport is considered, @fig-top10busiest_originating shows that the overwhelming majority of *all* round-trip routes originated out of Atlanta (`ATL`), with Dallas-Fort Worth (`DFW`) and Charlotte (`CLT`) coming in 2nd and 3rd.
...
As shown below in @fig-top10busiest_both, `Delta` (`DL`) had the greatest number of 1Q2019 flights by far, the majority of which originated out of its main hub in `Atlanta (ATL)`.
...
As seen in @fig-top10profit_route_carrier_probe, the following top 10 overall routes do have significant differences across carriers:
