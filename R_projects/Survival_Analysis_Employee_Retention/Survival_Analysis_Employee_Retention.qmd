...
::: callout-important
## The first section is a detailed overview of survival analysis and some of the underlying math. If you prefer, feel free to skip the technical stuff and jump straight to the math-free case study and R code!
:::
...
::: callout-caution
## Work in Progress! This is a temporary placeholder section for the math underlying the Kaplan Meier model.
:::

**Kaplan-Meier (KM) Survival Curves** allow us to estimate the **cumulative probability of** **retaining over time** (e.g. the probability of retaining past each tenure day after-hire).

-   For each time point where a termination was observed, the KM curve estimates the *proportion of employees that can be expected to retain (survive) past that time point.*

-   When we say "***cumulative***", we mean that the probability of retaining through Day 10 combines the probabilities of first surviving Day 1, then Day 2, ... , all the way through Day 10.
...
The **Cox Proportional Hazards (CPH)** regression model allows us to assess how different risk factors **increase or decrease the likelihood of an employee terminating**.

-   Because termination and retention are *complementary*, Cox models also give us the ability to **predict retention probabilities and times,** and to generate **retention curves**.

-   Cox models estimate **odds ratios** (also called **hazard ratios**), which estimate **how likely employees in one subgroup are to terminate** relative to employees in another subgroup.

::: callout-note
## \*\*\*Note: Cox models require an assumption of proportional hazards over time. At a high level this just means that if Denver employees have a 4x greater risk of termination than Charlotte employees, then we assume that 4:1 risk ratio is constant over all tenure time points after hire.
:::
...
### SEPARATE THEME FOR COMPETING RISKS
theme_bw() +
...
# OVERVIEW: Headcount data has monthly headcounts by grouping vars
...
## (Time = time observed, Status = event occurred or not, censors denoted via +)

# OVERALL SURVIVAL OBJECT (Time, Status): ANY TERM TYPE/EVENT
dat1surv |> 
...
# Summary - default survival table (not usable as DF)
...
(pc1facility + pc1hireclass) / (pc1manager + pc1payband)


# Adjusted to fit patchwork better
